...
###   litbiacore.org and litbia.org and compares them.
...
WORKINGDIR="/tmp/litbia_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://litbiacore.org"
HOST2="https://litbia.org"
BASEDIR="/bin/"
VERSIONPREFIX="litbia-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'litbia-',
...
if [ $? -ne 0 ]; then
   echo "litbia.org failed to provide signature file, but litbiacore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "litbia.org and litbiacore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the LitBia Core binary release signing key installed?"
   fi
